...
export function PortalWorkspace({ mode }: { mode: 'hospital' | 'patient' }) {
  ...
  useEffect(() => {
    ...
    const url = new URL(window.location.href); url.searchParams.set('patient', selectedId); window.history.replaceState(null, '', url.pathname + url.search);
  ...
  function patientSaved(saved: Patient) {
    ...
    setSelectedId(saved.id); setEditPatient(null); setNotice('Đã lưu hồ sơ chung. Góc nhìn bệnh nhân dùng cùng thông tin này.');
  }
  ...
  return <div className={`mp-app ${hospital ? 'mp-hospital' : 'mp-patient'}`}>
    ...
    <nav className="mp-mobile-links" aria-label="Trang dữ liệu và góp ý"><Link href="/data"><Database size={16} />Dữ liệu & Supabase</Link><Link href="/feedback"><MessageSquare size={16} />Yêu cầu chỉnh sửa</Link></nav>
    <ErrorBox message={error} />{error && <div className="mp-recovery"><Button variant="outline" onClick={() => void load()}>Thử lại</Button><a href="/signin-with-chatgpt?return_to=/editor" target="_top">Đăng nhập để mở demo</a><Link href="/data">Xem kết nối dữ liệu</Link></div>}
