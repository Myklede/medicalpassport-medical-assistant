...
export function PortalWorkspace({ mode }: { mode: 'hospital' | 'patient' }) {
  ...
  return <div className={`mp-app ${hospital ? 'mp-hospital' : 'mp-patient'}`}>
    ...
    <nav className="mp-mobile-links" aria-label="Trang dữ liệu, góp ý và công cụ"><Link href="/data"><Database size={16} />Dữ liệu & Supabase</Link><Link href="/feedback"><MessageSquare size={16} />Yêu cầu chỉnh sửa</Link><Link href="/records"><History size={16} />Hồ sơ trước đây</Link><Link href="/medications"><Pill size={16} />Đối chiếu thuốc</Link><Link href="/wounds"><Activity size={16} />Wound Lab</Link><Link href="/therapy"><HeartPulse size={16} />Motion Lab</Link></nav>
