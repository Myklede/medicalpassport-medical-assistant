...
export function PortalWorkspace({ mode }: { mode: 'hospital' | 'patient' }) {
  ...
  return <div className={`mp-app ${hospital ? 'mp-hospital' : 'mp-patient'}`}>
    ...
    <nav className="mp-mobile-links" aria-label="Trang dữ liệu và góp ý"><Link href="/data"><Database size={16} />Dữ liệu & Supabase</Link><Link href="/feedback"><MessageSquare size={16} />Yêu cầu chỉnh sửa</Link><Link href="/records"><History size={16} />Hồ sơ trước đây</Link><Link href="/wounds"><Activity size={16} />Wound Lab</Link><Link href="/therapy"><HeartPulse size={16} />Motion Lab</Link></nav>
    {data && data.storage.provider !== 'supabase' && <div className="mp-storage-warning" role="status">Hồ sơ đang ở bộ lưu demo, chưa được ghi vào Supabase. <Link href="/data">Hoàn tất kết nối dự án của bạn ↗</Link></div>}<ErrorBox message={error} />{error && <div className="mp-recovery"><Button variant="outline" onClick={() => void load()}>Thử lại</Button><a href="/signin-with-chatgpt?return_to=/editor" target="_top">Đăng nhập để mở demo</a><Link href="/data">Xem kết nối dữ liệu</Link></div>}
