...
export function PortalWorkspace({ mode }: { mode: 'hospital' | 'patient' }) {
  ...
  return <div className={`mp-app ${hospital ? 'mp-hospital' : 'mp-patient'}`}>
    <aside className="mp-rail"><Link className="mb-10 inline-flex rounded-xl px-2 text-white focus-visible:outline-2 focus-visible:outline-blue-400" href="/"><MediPassBrand /></Link><p className="mp-rail-label">KHÔNG GIAN CHĂM SÓC</p><nav><Link href="/editor" className={hospital ? 'active' : ''}><Stethoscope />Cổng bệnh viện</Link><Link href={selectedId ? `/patient?patient=${selectedId}` : '/patient'} className={!hospital ? 'active' : ''}><FileHeart />Góc nhìn bệnh nhân</Link><Link href="/feedback"><MessageSquare />Yêu cầu chỉnh sửa</Link><Link href="/data"><Database />Dữ liệu & Supabase</Link></nav><div className="mp-rail-divider" /><p className="mp-rail-label">CÔNG CỤ KHÁC</p><nav><Link href={selectedId ? `/records?patient=${encodeURIComponent(selectedId)}` : '/records'}><History />Hồ sơ trước đây</Link><Link href="/insurance"><ShieldCheck />Kiểm tra bảo hiểm</Link><Link href="/medications"><Pill />Đối chiếu thuốc</Link><Link href="/wounds"><Activity />Wound Lab</Link><Link href="/therapy"><HeartPulse />Motion Lab</Link></nav><div className="mp-rail-bottom"><ShieldCheck /><div><strong>Không gian demo riêng</strong><p>Bệnh nhân & bác sĩ giả lập</p></div></div></aside>
    <div className="mp-surface"><header className="mp-topbar"><div><Link href="/" className="mb-4 inline-flex rounded-xl focus-visible:outline-2 focus-visible:outline-blue-600 min-[1081px]:hidden"><MediPassBrand compact /></Link><p className="mp-eyebrow">{hospital ? 'DÀNH CHO NHÂN VIÊN Y TẾ' : 'HỒ SƠ CỦA BẠN'}</p><h1>{hospital ? 'Cổng bệnh viện' : 'Sổ sức khỏe'}</h1></div><div className="mp-top-actions"><Link className={`mp-storage-pill ${data?.storage.provider === 'supabase' ? 'connected' : ''}`} href="/data"><span />{data?.storage.provider === 'supabase' ? 'Supabase đã kết nối' : 'Demo · chờ Supabase'}</Link><Button variant="outline" onClick={() => void refresh()} aria-label="Làm mới hồ sơ"><RefreshCw /></Button><Link className="mp-button" href={`${hospital ? '/patient' : '/editor'}${selectedId ? `?patient=${selectedId}` : ''}`}><ArrowLeftRight size={16} />{hospital ? 'Xem phía bệnh nhân' : 'Vào cổng bệnh viện'}</Link></div></header>
    ...
    <nav className="mp-mobile-links" aria-label="Trang dữ liệu, góp ý và công cụ"><Link href="/data"><Database size={16} />Dữ liệu & Supabase</Link><Link href="/feedback"><MessageSquare size={16} />Yêu cầu chỉnh sửa</Link><Link href={selectedId ? `/records?patient=${encodeURIComponent(selectedId)}` : '/records'}><History size={16} />Hồ sơ trước đây</Link><Link href="/insurance"><ShieldCheck size={16} />Kiểm tra bảo hiểm</Link><Link href="/medications"><Pill size={16} />Đối chiếu thuốc</Link><Link href="/wounds"><Activity size={16} />Wound Lab</Link><Link href="/therapy"><HeartPulse size={16} />Motion Lab</Link></nav>
