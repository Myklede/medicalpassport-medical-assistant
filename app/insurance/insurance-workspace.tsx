    ...
    <main className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-30 border-b border-border bg-card/95 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1440px] items-center gap-3 px-4 sm:px-6 lg:px-8">
          <Link href="/editor" className="inline-flex size-11 items-center justify-center rounded-xl border border-border text-muted-foreground transition hover:bg-muted hover:text-foreground focus-visible:outline-2 focus-visible:outline-primary" aria-label="Quay lại cổng MediPass">
            <ArrowLeft className="size-4" />
          </Link>
          <Link href="/" className="flex min-h-11 items-center rounded-xl focus-visible:outline-2 focus-visible:outline-primary" aria-label="MediPass · Trang chủ"><MediPassBrand compact subtitle="INSURANCE" /></Link>
          <div className="ml-auto hidden items-center gap-2 text-sm text-slate-500 dark:text-slate-400 sm:flex"><LockKeyhole className="size-4 text-blue-700 dark:text-blue-400" />SBC được lưu trong kho tài liệu riêng</div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        <div className="flex flex-col gap-4 border-b border-slate-200 pb-7 dark:border-slate-800 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <Badge variant="outline" className="border-blue-200 bg-blue-50 text-blue-800 dark:border-blue-800 dark:bg-blue-950 dark:text-blue-200"><ShieldCheck className="size-3.5" />CÔNG CỤ DEMO</Badge>
        ...
        <section className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]" aria-label="Thiết lập phân tích SBC">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div><p className="text-sm font-semibold text-blue-700 dark:text-blue-300">BƯỚC 1</p><h2 className="mt-1 text-xl font-semibold">Chọn tài liệu SBC</h2></div>
            ...
            <form className="mt-5" onSubmit={uploadDocument}>
              ...
              {file && <Button type="submit" disabled={uploading} className="mt-3 h-11 w-full rounded-xl bg-primary hover:bg-primary/90">{uploading ? <><Loader2 className="size-4 animate-spin" />Đang đọc và lưu SBC…</> : <><UploadCloud className="size-4" />Lưu tài liệu vào thư viện</>}</Button>}
            </form>
                ...
                {documents.map((document) => {
                  ...
                  return <div key={document.id} className={`rounded-2xl border p-4 transition ${selected ? 'border-blue-400 bg-blue-50 ring-2 ring-blue-100 dark:border-blue-700 dark:bg-blue-950/30 dark:ring-blue-950' : 'border-slate-200 hover:border-slate-300 dark:border-slate-700 dark:hover:border-slate-600'}`}>
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3 border-t border-slate-200/70 pt-3 dark:border-slate-700">
                      <Badge variant="outline" className={document.extraction_status === 'document-text' ? 'border-blue-200 text-blue-800 dark:border-blue-800 dark:text-blue-200' : 'border-amber-200 text-amber-800 dark:border-amber-800 dark:text-amber-200'}>{document.extraction_status === 'document-text' ? 'Đã đọc văn bản' : 'Dữ liệu mô phỏng'}</Badge>
                      <Link href={document.download_url} className="inline-flex items-center gap-1.5 text-sm font-semibold text-blue-700 hover:text-blue-900 dark:text-blue-300 dark:hover:text-blue-100"><Download className="size-4" />Tải xuống</Link>
                    </div>
                  </div>;
                })}
              </div>
          ...
          <form className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6" onSubmit={runAnalysis}>
            ...
            <Button type="submit" disabled={!selectedDocumentId || condition.trim().length < 5 || analyzing} className="mt-6 h-12 w-full rounded-xl bg-primary text-base hover:bg-primary/90">{analyzing ? <><Loader2 className="size-4 animate-spin" />Đang phân tích điều khoản…</> : <><Sparkles className="size-4" />{selectedDocumentId ? 'Phân tích quyền lợi' : 'Chọn SBC để phân tích'}</>}</Button>
  ...
  return <section id="insurance-result" className="scroll-mt-6 mt-7 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900" aria-labelledby="insurance-result-title">
    <div className="border-b border-slate-200 p-5 dark:border-slate-800 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div><p className="text-sm font-semibold text-blue-700 dark:text-blue-300">KẾT QUẢ ƯỚC TÍNH</p><h2 id="insurance-result-title" className="mt-1 text-2xl font-semibold">{result.service}</h2><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">SBC: {analysis.document_name} · {formatDate(analysis.created_at)}</p></div>
