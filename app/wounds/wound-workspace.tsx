    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#f8fbfb_0%,#eef5f4_48%,#f8fafc_100%)] text-slate-950">
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1440px] items-center gap-3 px-4 sm:px-6 lg:px-8">
          ...
          <div className="ml-auto hidden items-center gap-2 rounded-full border border-violet-200 bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-800 sm:flex">
            <Sparkles className="size-3.5" /> Model adapter ready
          </div>
        </div>
      </header>
        ...
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.6fr)]">
          <div className="overflow-hidden rounded-3xl bg-slate-950 p-6 text-white shadow-xl shadow-slate-950/10 sm:p-8">
            ...
            <h1 className="mt-5 max-w-3xl text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">
              Track change over time without pretending a photo is a diagnosis.
            </h1>
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-300">
              Each capture joins the same wound case, the symptoms reported that day, and relevant saved health history. A trained image model can be connected later through the versioned adapter.
            </p>
          ...
          <aside className="space-y-5">
            ...
            <article className="rounded-3xl border border-violet-200 bg-violet-50 p-5 sm:p-6">
              <div className="flex items-center gap-2 text-violet-900">
                ...
                <h2 className="text-sm font-semibold">What the model does today</h2>
              </div>
              <p className="mt-3 text-sm leading-6 text-violet-950/75">
                {data?.model.message ?? 'Checking model status…'}
              </p>
