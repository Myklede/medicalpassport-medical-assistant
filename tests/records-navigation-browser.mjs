...
async function recordsHeader(target = page, patientId = '') {
  ...
  assert.equal(await link.innerText(), 'Clinical portal');
  ...
  const home = target.getByRole('link', { name: 'Back to MediPass lobby', exact: true });
  assert.equal(await home.getAttribute('href'), '/');
  await target.getByText('Previous records · separate module', { exact: true }).first().waitFor();
  return link;
}
...
try {
  ...
  for (const width of [1440, 390, 360]) {
    for (const theme of ['light', 'dark']) {
      ...
      await page.goto(base + '/records');
      await recordsHeader();
      await page.getByRole('link', { name: 'Back to MediPass lobby', exact: true }).click();
  ...
  const frameElement = page.locator('iframe[title="MediPass mobile preview"]');
