...
export default function WoundAnalyzer() {
  ...
  return <main className="wound-theme min-h-screen bg-slate-50 text-[#0F172A] transition-colors dark:bg-slate-950 dark:text-slate-100" data-testid="wound-analyzer" data-ready={ready} data-mode={developerMode ? 'developer' : 'patient'}>
    <header className="border-b border-slate-200 bg-white/95 dark:border-slate-800 dark:bg-slate-900/95">
      <nav className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-8" aria-label="Wound Lab navigation">
        <Link href="/" className="inline-flex rounded-xl focus-visible:outline-2 focus-visible:outline-blue-600"><MediPassBrand /></Link>
        <div className="flex flex-wrap gap-4 text-sm"><Link href="/patient" className="inline-flex min-h-11 items-center gap-2 text-slate-600 hover:text-blue-600 dark:text-slate-300"><Stethoscope className="size-4" />Medical Passport</Link><Link href="/wounds/history" className="inline-flex min-h-11 items-center gap-2 text-slate-600 hover:text-blue-600 dark:text-slate-300">History & captures<ArrowUpRight className="size-4" /></Link></div>
      </nav>
    </header>
    <div className="mx-auto max-w-7xl px-4 pb-32 pt-8 sm:px-8 sm:pt-10">
      <section className="mb-7 grid items-end gap-6 lg:grid-cols-[1fr_auto]" data-annotate="wound-analyzer-intro" data-annotation-label="Wound tracking and clinical history">
        <div><p className="mb-3 flex items-center gap-2 text-xs font-semibold tracking-[0.16em] text-blue-600 dark:text-blue-300"><HeartPulse className="size-4" />WOUND LAB · PRECISION WOUND CARE</p><h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">Every capture adds context to your progress.</h1><p className="mt-4 max-w-2xl text-base leading-7 text-slate-600 dark:text-slate-300">Track a wound over time by combining images with blood glucose, medical conditions, and care history.</p></div>
        <div className="flex items-center gap-3 rounded-2xl border border-blue-100 bg-white px-4 py-3 dark:border-blue-950 dark:bg-slate-900"><ShieldCheck className="size-5 text-blue-600 dark:text-blue-300" /><div><p className="text-sm font-semibold">Saved throughout the care episode</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Reload · add captures · delete on demand</p></div></div>
      </section>
      ...
      <section className="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
          <div><p className="flex items-center gap-2 font-semibold">{developerMode && <FlaskConical className="size-4 text-blue-600" />}{developerMode ? 'Developer Mode' : 'Patient Mode'}</p><p className="mt-1 text-xs leading-6 text-slate-500 dark:text-slate-400">{developerMode ? 'Sample profiles · select any capture to inspect the pipeline' : 'Simulated patient account · fixed profile'}</p></div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-5 px-5 py-5">
          <div className="flex min-w-0 items-center gap-3"><div className="grid size-12 shrink-0 place-items-center rounded-2xl bg-[#0F172A] text-lg font-semibold text-white">{patient.display_name.split(' ').map(word => word[0]).slice(0, 2).join('')}</div><div><h2 className="font-semibold">{patient.display_name}</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Age {patient.age} · blood type {patient.blood_type} · {patient.patient_id}</p></div></div>
          {developerMode && <label className="min-w-0 text-xs font-medium text-slate-500 dark:text-slate-400">Select a research profile<select aria-label="Select simulated patient" disabled={busy} value={selectedPatient} onChange={event => setSelectedPatient(event.target.value)} className="mt-2 block min-h-11 w-full min-w-0 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:ring-2 focus:ring-blue-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">{MOCK_WOUND_PATIENTS.map(profile => <option key={profile.patient_id} value={profile.patient_id}>{profile.display_name} · {profile.patient_id}</option>)}</select></label>}
          <div className="flex flex-wrap gap-2 text-xs font-medium"><span className="rounded-full bg-blue-50 px-3 py-2 text-blue-700 dark:bg-blue-950 dark:text-blue-200">HbA1c {patient.hba1c_level}%</span><span className={'rounded-full px-3 py-2 ' + (patient.has_diabetes_type_2 ? 'bg-amber-50 text-amber-800 dark:bg-amber-950 dark:text-amber-200' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300')}>{patient.has_diabetes_type_2 ? 'Type 2 diabetes recorded' : 'No type 2 diabetes recorded'}</span></div>
        </div>
      </section>
      ...
      <p className="mt-6 text-xs leading-6 text-slate-500 dark:text-slate-400">MediPass Wound Lab · Sample clinical profiles are stored separately from uploaded images. AI outputs are research estimates from models trained with synthetic images and require review by a healthcare professional alongside direct examination.</p>
    </div>
  </main>;
}
