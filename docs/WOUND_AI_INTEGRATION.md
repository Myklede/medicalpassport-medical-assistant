...
The deployed Sites build no longer needs a reachable laptop or Python port 8000
for uploaded PNG/JPEG inference. The browser loads pinned same-origin ONNX Runtime
Web/WASM plus a 24.6 MB int8 convolution-weight derivative of the supplied FUSd
U-Net and the 125 KB fp32 synthetic tissue model. It performs ImageNet
normalization, the same >0.35 mask threshold, dominant-component cleanup and
inside-mask tissue counting, then sends bounded results and 256 px pipeline rasters
to the same-origin cloud session API. D1 stores metadata/results and R2 stores the
original private image. A saved pending cloud image can be retried from its stored
bytes in the browser.

The first custom image downloads about 39 MB of model/runtime assets. The browser
artifacts are reproducibly generated by `scripts/export-wound-browser-models.py`;
their hashes and 0.936779 fixture-mask IoU versus fp32 ONNX are recorded in
`public/wound-models/models.json`. This is research execution, not clinical
validation. The hosted path does not run the late-fusion risk head.
...
laptop's service; it instead uses the bounded browser ONNX path described above.
Production patient authentication remains separate work.
